...
def getFilledSmallBods():
    ...
    smallBods = Bod.Bod.findAllBodItems(selectedBodGraphic, selectedBodHue, True)
    fullSmallBods = []
    for smallBod in smallBods:
        if Bod.Bod.isFilled(smallBod, False):
...
def main():
    ...
    while len(filledSmallBods) > 0:
        ...
        Util.Util.moveItem(filledSmallBod.Serial, tailorNpcSerial)
